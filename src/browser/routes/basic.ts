...
// 🔒 VOTAL.AI Security Fix: Missing authentication/authorization on privileged browser control endpoints [CWE-306] - CRITICAL
  ...
  app.post("/start", async (req, res) => {
    if (process.env.BROWSER_CONTROL_TOKEN && req.query.token !== process.env.BROWSER_CONTROL_TOKEN) return jsonError(res, 401, "unauthorized"); // require token for privileged endpoint
    const profileCtx = getProfileContext(req, ctx);
    if ("error" in profileCtx) {
      return jsonError(res, profileCtx.status, profileCtx.error);
    }
